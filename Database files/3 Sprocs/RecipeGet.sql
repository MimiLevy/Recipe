...
begin
	...
	select @RecipeId = isnull(@RecipeId,0), @RecipeName = isnull(@RecipeName,''), @CookbookName = isnull(@CookbookName,''), @CuisineTypeDesc = isnull(@CuisineTypeDesc,''), @All = isnull(@All,0)
	...
	or(@CuisineTypeDesc <> '' and ct.CuisineTypeDesc like '%' + @CuisineTypeDesc + '%')
	or @All = 1
...
exec RecipeGet @CuisineTypeDesc = 'American'
